...
Qrafty.s("MouseWheel", Qrafty.extend.call(new Qrafty.__eventDispatcher(), {
	...
	prepareEvent: function (e) {
		...
		if (!mouseSystem) this._mouseSystem = mouseSystem = Qrafty.s("Mouse");
		...
		Qrafty.translatePointerEventCoordinates(e, evt);

		// augment mouse event with target entity
		evt.target = mouseSystem.mouseObjs ? Qrafty.findPointerEventTargetByComponent("Mouse", e) : null;
	...
	dispatchEvent: function (e) {
		...
		// trigger event
		Qrafty.trigger("MouseWheelScroll", evt);
	}
}), {}, false);
 ...
 */
Qrafty.s("Mouse", Qrafty.extend.call(Qrafty.extend.call(new Qrafty.__eventDispatcher(), {
	normedEventNames: {
		"mousedown": "MouseDown",
		"mouseup": "MouseUp",
	...
	prepareEvent: function (e) {
		...
		if (typeof e.which === "undefined") {
			evt.mouseButton = (e.button < 2) ? Qrafty.mouseButtons.LEFT : ((e.button === 4) ? Qrafty.mouseButtons.MIDDLE : Qrafty.mouseButtons.RIGHT);
		} else {
			evt.mouseButton = (e.which < 2) ? Qrafty.mouseButtons.LEFT : ((e.which === 2) ? Qrafty.mouseButtons.MIDDLE : Qrafty.mouseButtons.RIGHT);
		}
		...
		// augment mouse event with target entity
		evt.target = this.mouseObjs ? Qrafty.findPointerEventTargetByComponent("Mouse", e) : null;
...
}), Qrafty.__mouseStateTemplate), {}, false);
...
Qrafty.c("MouseDrag", {
	...
	_ondown: function (e) {
		if (e.mouseButton !== Qrafty.mouseButtons.LEFT) return;
		this.startDrag(e);
	},
	...
	startDrag: function (e) {
		...
		// TODO: remove this and instead lock on pointer control events in future
		Qrafty.s("Mouse").bind("MouseMove", this._ondrag);
		Qrafty.s("Mouse").bind("MouseUp", this._onup);

		// if event undefined, use the last known position of the mouse
		this.trigger("StartDrag", e || Qrafty.s("Mouse").lastMouseEvent);
		return this;
	},
	...
	stopDrag: function (e) {
		...
		// TODO: remove this and instead lock on pointer control events in future
		Qrafty.s("Mouse").unbind("MouseMove", this._ondrag);
		Qrafty.s("Mouse").unbind("MouseUp", this._onup);

		// if event undefined, use the last known position of the mouse
		this.trigger("StopDrag", e || Qrafty.s("Mouse").lastMouseEvent);
		return this;
	}
});
