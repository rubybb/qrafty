...
MouseButtonToggleInput.prototype = {
	isDown: function() {
		...
		if (!mouseSystem) this.mouseSystem = mouseSystem = Qrafty.s("Mouse");
...
KeyboardToggleInput.prototype = {
	isDown: function() {
		...
		if (!keyboardSystem) this.keyboardSystem = keyboardSystem = Qrafty.s("Keyboard");
...
Qrafty.s("Controls", {
	init: function () {
		...
		Qrafty.s("Mouse").bind("MouseDown", this.updateTriggers.bind(this));
		Qrafty.s("Mouse").bind("MouseUp", this.updateTriggers.bind(this));
		Qrafty.s("Mouse").bind("DoubleClick", this.updateTriggers.bind(this));
		Qrafty.s("Mouse").bind("Click", this.updateTriggers.bind(this));
	},
	...
	runEvents: function () { 
		...
		for (var d in this._dpads) {
			...
			if (dpad.x !== dpad.oldX || dpad.y !== dpad.oldY) {
				Qrafty.trigger("DirectionalInput", dpad.event);
			}
		}
	},
	...
	defineTriggerGroup: function(name, definition) {
		...
		Qrafty.trigger("ControlDefined", { type: "TriggerGroup", name: name });
	},
	...
	destroyTriggerGroup: function(name) {
		if (this._triggers[name]) {
			...
			Qrafty.trigger("ControlDestroyed", { type: "TriggerGroup", name: name });
		}
	},
	...
	defineDpad: function (name, definition, options) {
		...
		Qrafty.trigger("ControlDefined", { type: "Dpad", name: name });
	},
	...
	destroyDpad: function (name) {
		if (this._dpads[name]) {
			...
			Qrafty.trigger("ControlDestroyed", { type: "Dpad", name: name });
		}
	},
	...
	updateTriggerInput: function (trigger) {
		if (!trigger.active) {
			if (trigger.input.isActive()) {
				...
				trigger.active = true;
				Qrafty.trigger("TriggerInputDown", trigger);
			}
		} else {
			if (!trigger.input.isActive()) {
				trigger.active = false;
				Qrafty.trigger("TriggerInputUp", trigger);
				trigger.downFor = 0;
			}
		}
	},
