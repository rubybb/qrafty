...
Qrafty.extend({
	...
	audio: {
		...
		create: function (id, path) {
			...
			audio.id = id;
			audio.preload = "auto";
			audio.volume = Qrafty.audio.volume;
			audio.src = path;

			//create an asset and metadata for the audio element
			Qrafty.asset(path, audio);
			this.sounds[id] = {
				obj: audio,
				played: 0,
				volume: Qrafty.audio.volume
			};
		...
		add: function (id, url) {
			...
			if (arguments.length === 1 && typeof id === "object") {
				for (var i in id) {
					for (src in id[i]) {
						a = Qrafty.audio.create(i, id[i][src]);
						if (a){
							break;
						}
					}
			...
			if (typeof id === "string") {
				if (typeof url === "string") {
					a = Qrafty.audio.create(id, url);
				}

				if (typeof url === "object") {
					for (src in url) {
						a = Qrafty.audio.create(id, url[src]);
						if (a)
							break;
					}
				}
		...
		play: function (id, repeat, volume) {
			if (repeat === 0 || !Qrafty.support.audio || !this.sounds[id])
				return;
			...
			c.volume = s.volume = s.obj.volume = volume || Qrafty.audio.volume;
		...
		remove: function (id) {
			...
			var s, filename, audioFolder = Qrafty.paths().audio;

			if (!id) {
				for (var i in this.sounds) {
					s = this.sounds[i];
					filename = s.obj.src.split("/").pop();
					Qrafty.audio.stop(id);
					delete Qrafty.assets[audioFolder + filename];
					delete Qrafty.assets[s.obj.src];
					delete Qrafty.audio.sounds[id];
				}
				return;
			}
			if (!this.sounds[id])
         ...
         */
		pause: function (id) {
			if (!Qrafty.support.audio || !id || !this.sounds[id])
				return;
			var c;
			for (var i in this.channels) {
				c = this.channels[i];
