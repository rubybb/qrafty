...
Qrafty.c("Supportable", {
	...
	_detectGroundTick: function() {
		var groundComp = this._groundComp,
			ground = this._ground,
			overlap = Qrafty.rectManager.overlap,
			area;
		...
		if (ground) {
			...
			if (!(ground.__c[groundComp] && Qrafty(ground[0]) === ground && overlap(garea, area))) {
				this._ground = null;
		...
		if (!ground) {
			var obj, oarea,
				results = Qrafty.map.unfilteredSearch(area),
				i = 0,
				l = results.length;
