...
Qrafty.c("2D", {
	...
	init: function () {
		...
		this._entry = Qrafty.map.insert(this);
	...
	events: {
		...
		"Unfreeze":function(){
			this._entry = Qrafty.map.insert(this, this._entry);
		}
	}, 
	...
	_setPosition: function(x, y) {
		...
		var old = Qrafty.rectManager._pool.copy(this);
		var mbr = this._mbr;
		if (mbr) {
			mbr._x -= this._x - x;
			mbr._y -= this._y - y;
		...
		Qrafty.rectManager._pool.recycle(old);
	},
...
Qrafty.polygon.prototype = {
	...
	clone: function() {
		...
		return new Qrafty.polygon(this.points.slice(0));
	},
 ...
 * (don't include the absolute values as it will automatically calculate this).
 */
Qrafty.circle = function (x, y, radius) {
	this.x = x;
	this.y = y;
	this.radius = radius;
...
Qrafty.matrix = function (m) {
	this.mtx = m;
	this.width = m[0].length;
	this.height = m.length;
};

Qrafty.matrix.prototype = {
	x: function (other) {
		...
		return new Qrafty.matrix(result);
	},
