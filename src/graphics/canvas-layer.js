 ...
 */
Qrafty._registerLayerTemplate("Canvas", {
	type: "Canvas",
	...
	layerInit: function () {
		...
		var zoom = Qrafty.viewport._scale;
		if (zoom !== 1)
	...
	_drawDirtyCells: function (view) {
		var viewportRect = this._viewportRect(), // this updates the viewportRect for later cached use
			...
			integerBounds = Qrafty.rectManager.integerBounds,
			ctx = this.context;
		var i, l;
	...
	_drawRect: function(rect) {
		...
		q = Qrafty.map.search(searchRect);
	...
	_createDirtyCells: function (view) {
		...
		var viewKeys = Qrafty.HashMap.key(view, this._viewKeys);
	...
	_createDirtyRects: function() {
		var cellsize = Qrafty.HashMap.cellsize(),
			dirtyCells = this._dirtyCells,
			dirtyRects = this._dirtyRects;
