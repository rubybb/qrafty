...
Qrafty.c("DOM", {
	...
	init: async function () {
		...
		if (!this._customLayer){
			this._attachToLayer( Qrafty.s("DefaultDOMLayer") );
		}
	...
	draw: function () {
		...
		if (Qrafty.support.css3dtransform && !this.avoidCss3dTransforms) {
		...
		if (this._mbr) {
			...
			if (Qrafty.support.css3dtransform) trans.push("rotateZ(" + this._rotation + "deg)");
	...
	_setCssProperty: function(style, key, val) {
		key = Qrafty.domHelper.camelize(key);
	...
	css: function (obj, value) {
		...
		if (typeof obj === "object") {
			...
			if (value) {
				...
				return Qrafty.domHelper.getStyle(elem, obj);
			}
		}
