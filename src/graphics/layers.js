...
Qrafty.extend({
	...
	_commonLayerProperties: {
		...
		init: function() {
			...
			this.trigger("PixelartSet", Qrafty._pixelartEnabled);

			Qrafty._addDrawLayerInstance(this);
		},
		...
		remove: function() {
			...
			Qrafty._removeDrawLayerInstance(this);
		},
	...
	createLayer: function createLayer(name, type, options) {
		...
		Qrafty.s(name, layerTemplate, options);
		Qrafty.c(name, {
			init: function () {
				...
				this._attachToLayer(Qrafty.s(name));
			},
