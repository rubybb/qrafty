...
Qrafty.defaultShader("Sprite", new Qrafty.WebGLShader(
	shaders_sprite_vert,
	shaders_sprite_frag,
	[
...
Qrafty.extend({
	...
	sprite: function (tile, tileh, url, map, paddingX, paddingY, paddingAroundBorder) {
		...
		img = Qrafty.asset(url);
		if (!img) {
			img = new Image();
			img.src = url;
			Qrafty.asset(url, img);
			img.onload = function () {
				//all components with this img are now ready
				for (var spriteName in map) {
					Qrafty(spriteName).each(markSpritesReady);
				}
			};
		}
...
Qrafty.c("Sprite", {
	...
	_setupSpriteImage: function(layer) {
		...
		if (layer.type === "WebGL"){
			this._establishShader(this.__image, Qrafty.defaultShader("Sprite"));
